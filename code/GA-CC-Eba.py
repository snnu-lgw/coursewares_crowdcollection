...
def fitness1(N, M, unit):
    ...
    for j in range(N):
        if Sum == 0:
            ...
            continue
        Global_variable.b[j] = Global_variable.B * unit[j] / Sum
...
def search_answer(N, M, population,flag):
    ...
    for i in range(M):
        if flag == 0:
            a[i], _, _, _ = fitness1(N, M, population[i])
        else:
            a[i] = fitness2(N, M, population[i])
    ...
    if flag == 0:
        ...
        Diversity, Average_Courseware_Support_Rate, Difference, Task_num = fitness1(N, M, population[max_id])
    else:
        Diversity, Average_Courseware_Support_Rate, Difference = fitness2(N, M, population[max_id])
...
def main(N, M, T, P_cross, P_mutation,flag):
    ...
    for i in range(all_len):
        all.append(Global_variable.encode(N))  # 生成all_len个随机01串,每个01串的长度为N，即判断N个课程是否选择
    # print(all)
...
#############################################################################################################################
    ...
    Global_variable.av_B = int(sys.argv[6])
    init_var()  # 初始化变量

    print("分布选择为:", Distribution_Choose, 'beta == ', "预算B == ", Global_variable.B)
    ...
    # print(Global_variable.lower_bound(mp,0.6))
#############################################################################################################################
    cnt = 0


    s1 = time.time()
    main(Global_variable.N, 200, Age, 0.6, 0.05,0)
    s2 = time.time()
    t1 = s2 - s1
